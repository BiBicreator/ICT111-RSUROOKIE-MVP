...
| Status update | Can records be edited without control? | Only administrators can update building information and chatbot data. | Medium | Restrict editing privileges to authorized administrators and log updates where possible. | Member 2 |
| Public links | Does a public link expose data that should be private? | Public pages only display campus information. Admin pages are not linked publicly. | Low | Keep administrator pages protected and avoid exposing management URLs. | Member 2 |
